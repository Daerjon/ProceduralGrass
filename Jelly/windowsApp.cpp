...
PumaBase::PumaBase(HINSTANCE hInst)
	: dx_app(hInst, 1280, 720, L"Shader Demo"), m_loader(m_device), m_layouts(m_device), m_camera({ 0,0,0 },0.01f, 50.0f, 5),
	  m_frustrum(get_window().client_size(), XM_PIDIV4, 0.5f, 85.0f), m_gui(m_device, get_window())
{
}

std::optional<LRESULT> PumaBase::process_message(windows::message const &msg)
{
...
size_t PumaBase::addModelFromFile(const std::string& path)
{
	m_models.push_back(make_unique<Model>(m_loader.LoadFromFile(path, m_layouts)));
	return m_models.size() - 1;
}

std::tuple<size_t, ModelDescription> mini::puma::PumaBase::addModelFromTxtFile(const std::string& path)
{
	auto t = m_loader.loadFromTxtFile(path, m_layouts);
	auto& [m1, md] = t;
	m_models.push_back(make_unique<Model>(std::move(m1)));

	return { m_models.size() - 1 ,md };
}

size_t PumaBase::addModelFromString(const std::string& model, bool smoothNormals)
{
	m_models.push_back(make_unique<Model>(m_loader.LoadFromString(model, m_layouts, smoothNormals)));
	return m_models.size() - 1;
}

size_t mini::puma::PumaBase::addModelFromBuffers(std::vector<float> vert, std::vector<float> norm, std::vector<unsigned short> ind)
{
	m_models.push_back(make_unique<Model>(m_loader.CreateModel(vert, norm, ind, m_layouts)));
	return m_models.size() - 1;
}

size_t mini::puma::PumaBase::addModelParticleSystem()
{
	m_models.push_back(make_unique<Model>(m_loader.CreateParticleSystem(1, m_layouts)));
	return m_models.size() - 1;
}

size_t PumaBase::addPass(const std::wstring& vsShader, const std::wstring& psShader)
{
	m_passes.emplace_back(m_device, m_variables, &m_layouts, vsShader, psShader);
	return m_passes.size() - 1;
}

size_t PumaBase::addPass(const std::wstring& vsShader, const std::wstring& gsShader,
...
void PumaBase::addRasterizerState(size_t passId, const directx::rasterizer_info& desc)
{
	m_passes[passId].AddEffect(make_unique<RasterizerEffect>(m_device.CreateRasterizerState(desc)));
}

void PumaBase::addModelToPass(size_t passId, size_t modelId)
{
	m_passes[passId].AddModel(m_models[modelId].get());
}

void PumaBase::copyRenderTarget(size_t passId, std::string dstTexture)
{
	pass(passId).EmplaceEffect<CopyRenderTargetEffect>(m_variables.GetTexture(dstTexture));
}

void PumaBase::copyDepthBuffer(size_t passId, std::string dstTexture)
{
