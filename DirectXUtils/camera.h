...
namespace mini::directx
{
	...
	public:
		...
		explicit fps_camera(DirectX::XMFLOAT3 target, float min_distance, float max_distance = std::numeric_limits<float>::max(),
			float distance = 0.0f) noexcept
			: orbit_camera{ target, min_distance, max_distance, distance }
		{ }
